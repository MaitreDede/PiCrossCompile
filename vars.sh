...
QEMU_OPTS=(-kernel kernel-qemu -cpu arm1176 -m 256 -M raspi2 -no-reboot -serial stdio -append 'root=/dev/mmcblk0p2 earlyprintk rootfstype=ext4 console=ttyAMA0 rw' -drive file=${IMAGE},format=raw -net user,hostfwd=tcp::10022-:22,hostfwd=tcp::18069-:8069 -net nic)
if [ -z ${DISPLAY:-} ] ; then
    QEMU_OPTS+=(-nographic)
fi
